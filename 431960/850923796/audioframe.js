...
function AudioFrame( opts )
{
	...
	this.update = function( data, isFake )
	{
		...
		try {
			...
			this.audioPeakAverage = this.audioPeakAverage * this.config.normalizeFactor + maxValue * (1-this.config.normalizeFactor);
			...
			if( !isFake ) {
				...
				this.audioDataNormalized.multiply( animateWhenSilentHeight * 3 );
			}
